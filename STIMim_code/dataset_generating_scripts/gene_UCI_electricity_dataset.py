...
from dataset_generating_scripts.data_processing_utils import (
    ...
    add_artificial_mask,
    saving_into_h5,
    add_structural_mask,#structural mask
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mask_mode",
        type=str,
        choices=["random", "structural"],#artificial mask
        default="structural",
        help="Type: random or structural"
    )    
    ...
    logger = setup_logger(
        ...
        os.path.join(dataset_saving_dir, f"dataset_generating_{args.mask_mode}.log"),
    ...
    if args.mask_mode == "random":
        ...
        val_set_dict  = add_structural_mask(val_set_X, args.artificial_missing_rate, "val", seed=42)
        test_set_dict = add_structural_mask(test_set_X, args.artificial_missing_rate, "test", seed=42)
    ...
    saving_into_h5(dataset_saving_dir, processed_data, classification_dataset=False, mask_mode=args.mask_mode)#rename
    #pickle_dump(scaler, os.path.join(dataset_saving_dir, 'scaler'))
    pickle_dump(scaler, os.path.join(dataset_saving_dir, f'scaler_{args.mask_mode}'))
