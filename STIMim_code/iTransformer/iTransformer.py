...
def FeedForward(dim, mult=4, dropout=0., use_mamba=True):
    if use_mamba:
        from mamba_ssm.modules.mamba_simple import Mamba
        return Mamba(
            d_model=dim,
        ...
        *,
        ...
        for _ in range(depth):
            self.layers.append(ModuleList([
                ...
                FeedForward(dim, mult=ff_mult, dropout=ff_dropout, use_mamba=use_mamba),
                nn.LayerNorm(dim)
            ]))
        ...
        if use_mamba:
            self.mamba_preblock = Mamba(
                d_model=dim,
                d_state=4,
                expand=2
            )
        else:
            self.mamba_preblock = nn.Identity()
