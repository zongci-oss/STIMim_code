...
# from SoftmaxVAE2 import generate_multiple_samples
...
class LoadValTestDataset(LoadDataset):
    ...
    def __init__(self, file_path, set_name, seq_len, feature_num, model_type, hint_rate):
    ...
    def __getitem__(self, idx):
        ...
        elif self.model_type in ['STIMim']:
            # print("test missing_mask1 shape: ", self.missing_mask.shape, "\n" ,"test indicating_mask1 shape: ", self.indicating_mask.shape)
            # time.sleep(5)
            deltaPre = parse_deltaPre(self.X_hat[idx],self.missing_mask[idx],self.seq_len)
            H = parse_H(self.missing_mask[idx],self.hint_rate)
            sample = (
                torch.tensor(idx),
                torch.from_numpy(self.X_hat[idx].astype('float32')),
                torch.from_numpy(self.missing_mask[idx].astype('float32')),
                torch.from_numpy(H.astype('float32')),
...
class LoadTrainDataset(LoadDataset):
    ...
    def __init__(self, file_path, seq_len, feature_num, model_type, masked_imputation_task, hint_rate, miss_rate):
    ...
    def __getitem__(self, idx):
        ...
        if self.masked_imputation_task:
            ...
            '''
            indicating_mask = ((~np.isnan(X)) ^ (~np.isnan(X_hat))).astype(np.float32)
            # print("missing_mask1 shape: ", missing_mask.shape, "\n" ,"indicating_mask1 shape: ", indicating_mask.shape)
            ...
            # print("missing_mask2 shape: ", missing_mask.shape, "\n" ,"indicating_mask2 shape: ", indicating_mask.shape)
            # time.sleep(5)
            if self.model_type in ['Transformer', 'SAITS']:
                sample = (
                    torch.tensor(idx),
                ...
                H = parse_H(missing_mask,self.hint_rate)
                sample = (
                    torch.tensor(idx),
                    torch.from_numpy(X_hat.astype('float32')),
                    torch.from_numpy(missing_mask.astype('float32')),
...
class UnifiedDataLoader:
    def __init__(self, dataset_path, seq_len, feature_num, model_type, hint_rate, miss_rate, batch_size=1024, num_workers=4,
    ...
    def get_train_val_dataloader(self):
        self.train_dataset = LoadTrainDataset(self.dataset_path, self.seq_len, self.feature_num, self.model_type,
                                              self.masked_imputation_task, self.hint_rate, self.miss_rate)
        self.val_dataset = LoadValTestDataset(self.dataset_path, 'val', self.seq_len, self.feature_num, self.model_type, self.hint_rate)
    ...
    def get_test_dataloader(self):
        self.test_dataset = LoadValTestDataset(self.dataset_path, 'test', self.seq_len, self.feature_num,
                                               self.model_type, self.hint_rate)
